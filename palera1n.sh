...
function _network() {
    curl -s -m 1 https://github.com/g114-39/Applera1n-Files-Backup &>/dev/null
}
...
if [ ! -e "$dir"/gaster ]; then
    ...
    curl -sLO https://nightly.link/verygenericname/gaster/workflows/makefile/main/gaster-"$os-x86_64".zip
    unzip gaster-"$os-x86_64".zip
    mv gaster "$dir"/
    rm -rf gaster gaster-"$os-x86_64".zip
fi
...
if [ "$china" != "1" ]; then
    ...
    curl -LO https://raw.githubusercontent.com/g114-39/Applera1n-Files-Backup/refs/heads/main/ramdisk.tgz
    tar xf ramdisk.tgz
fi
...
if [ ! -f blobs/"$deviceid"-"$version".der ]; then
    ...
    echo "-- Ramdisk already created, trying to boot it --"
    ...
    if [ "$tweaks" = "1" ] && [ "$semi_tethered" = "1" ]; then
        echo "KILL USBMUXD AND THEN RESTART IT NOW!"
        sleep 5
    ...
    sleep 1
    if [[ "$deviceid" == *'iPhone8'* ]] || [[ "$deviceid" == *'iPad6'* ]] || [[ "$deviceid" == *'iPad5'* ]]; then
        pyimg4 im4p create -i work/kcache.patched2 -o work/kcache.im4p -f krnl --extra work/kpp.bin --lzss
    else
        pyimg4 im4p create -i work/kcache.patched2 -o work/kcache.im4p -f krnl --lzss
    fi
    sleep 1
...
url="https://raw.githubusercontent.com/g114-39/Applera1n-Files-Backup/refs/heads/main/patch"
target_dir="./"
...
url="https://raw.githubusercontent.com/g114-39/Applera1n-Files-Backup/refs/heads/main/com.bypass.mobileactivationd.plist"
target_dir="./"
...
if [ $? -eq 0 ]; then
    ...
    echo "plist can not be downloaded"
        ...
        curl -LO https://raw.githubusercontent.com/g114-39/Applera1n-Files-Backup/refs/heads/main/palera1n.ipa
        ...
        curl -L https://raw.githubusercontent.com/g114-39/Applera1n-Files-Backup/refs/heads/main/rootfs.zip -o rfs.zip
        unzip rfs.zip -d .
        unzip rootfs.zip -d .
        ...
        curl -L https://raw.githubusercontent.com/g114-39/Applera1n-Files-Backup/refs/heads/main/binpack.tar -o other/rootfs/jbin/binpack/binpack.tar
